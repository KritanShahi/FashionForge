...
const ProductDetail = () => {
  ...
  // Handle rating increment when love button is clicked
  const handleLoveClick = async () => {
    try {
      const response = await axios.post(`http://localhost:8080/api/products/${id}/love`);
  ...
  const handleAddComment = async () => {
    if (newComment) {
      try {
        const response = await axios.post(`http://localhost:8080/api/products/${id}/comments`, { user: 'You', text: newComment });
...
<Navbar></Navbar>
      ...
      <Details>
        ...
        <Rating>
          ...
          <LoveButton onClick={handleLoveClick}><StarBorderIcon></StarBorderIcon></LoveButton>
        </Rating>
